...
class SimulationTestNode(Node):
    ...
    def is_initialized(self):
        if (
            self.is_controller_msg
            and self.is_ekf_msg
            and self.is_imu_msg
            and self.is_joint_msg
        ):
...
def wait_for_initialization(node: SimulationTestNode, robot_name="robot"):
...
def speed_test(
    node: SimulationTestNode, test_name, v_x=0.0, v_y=0.0, v_yaw=0.0, robot_name="robot"
):
...
def sensors_readings_test(
    node: SimulationTestNode, events_to_check, robot_name="robot"
):
    events_dict = {
